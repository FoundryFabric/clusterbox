...
// TailscaleSpec configures the Tailscale enrolment section.
//
// When Enabled is true, AuthKey must be provided. Hostname overrides the
// device name registered in the tailnet; if empty, the OS hostname is used.
...
// AllowedDistros is the set of distro values accepted by Validate.
//
// An empty string is also valid and means "auto-detect".
var AllowedDistros = []string{"ubuntu", "flatcar"}
...
//
//   - When Distro is non-empty, it must be one of AllowedDistros.
...
//   - When Tailscale.Enabled is true, AuthKey must be non-empty.
//
...
func (s *Spec) Validate() error {
	...
	if s.Distro != "" && !distroAllowed(s.Distro) {
		return fmt.Errorf("distro %q is not allowed (want one of %v)", s.Distro, AllowedDistros)
	}
	...
	if ts := s.Tailscale; ts != nil && ts.Enabled {
		if ts.AuthKey == "" {
			return errors.New("tailscale: auth_key is required when enabled")
		}
	}
	return nil
}
